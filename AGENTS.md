...
- **Open beside** splits the current tab (one tab, two panes). Drag-reorder uses `pages.move` / `folders.move`. Space export writes `paper.json` plus files; Settings backup is the full app (no secret values). Page **delete** goes to trash (`deletedAt`, 30-day purge). Do not invent `$vars` / `{{}}` until 0.2.
